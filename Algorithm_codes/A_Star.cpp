...
void A_Star(string source,string goal)
{
    priority_queue<town>q;
    town t,w,v;
    t.name=source; t.cost=Distance[source][goal];
    q.push(t);
    vis[source]=true;
    ...
    while(q.empty()==false)
    {
        ...
        for(int i=0;i<Connected_Road[c].size();i++)
        {
            ...
            if(length[c]+dist[c][s]+Distance[s][goal]<length[s]+Distance[c][goal] or length[s]==-1)
            {
                length[s]=length[c]+dist[c][s];
                v.cost=length[s]+Distance[s][goal];
                v.name=s;
                q.push(v);
                parent[s]=c;
            }
